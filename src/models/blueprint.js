...
const generateLookupPaths = lookupPaths => {
	lookupPaths = lookupPaths || [];
	lookupPaths = lookupPaths.concat(Blueprint.defaultLookupPaths());
	return _.uniq(lookupPaths);
};

const dir = fullPath => fileExists(fullPath) ?
	fs.readdirSync(fullPath).map(fileName => path.join(fullPath, fileName))
	:[];
...
export default class Blueprint {
	...
	processFiles(locals) {
		...
		this.logger.writeDebug(`built file infos: ${fileInfos.length}`);
		const filesToWrite = fileInfos.filter(info => info.isFile());
		this.logger.writeDebug(`files to write: ${filesToWrite.length}`);
		filesToWrite.map(file => file.writeFile());
	}

	buildFileInfo(locals, file) {
		const mappedPath = this.mapFile(file, locals);
		this.logger.writeDebug(`mapped path: ${mappedPath}`);
	...
	*/
	install(options) {
		const logger = this.logger = options.logger;

		logger.writeInfo(`开始创建: ${options.originalBlueprintName} - ${options.entity.name}`);
